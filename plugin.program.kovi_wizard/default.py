...
#                                                                              #
#  Translated and modified by MICLIER                                      #
#                                                                              #
################################################################################
...
MYBUILDS         = os.path.join(BACKUPLOCATION, 'Meine_Builds', '')
...
def index():
	...
	addDir ('Konten sichern'     ,'savedata', icon=ICONSAVE,     themeit=THEME1)
	if HIDECONTACT == 'No': addFile('Support' ,'contact', icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		addDir ('Datensicherung Menu'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		...
		if len(match) >= 1:
			...
			if adultcount > 0:
				...
				addFile('Adult Builds im Menu anzeigen > Misc', '', icon=ICONBUILDS, themeit=THEME3)
			else:
...
def apkScraper(name=""):
	if name == 'kodi':
		...
		addFile("Offizielle Kodi Versionen", themeit=THEME1)
		rc = False
...
def apkMenu(url=None):
	if url == None:
		addDir ('Offizielle Kodi Versionen', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if not APKFILE == 'http://':
		if url == None:
...
def maintMenu(view=None):
	...
	if view == "tweaks" or SHOWMAINT == 'true': 
		if not ADVANCEDFILE == 'http://' and not ADVANCEDFILE == '':
			...
			addFile('Schnelle Konfiguration der AdvancedSettings.xml',    'autoadvanced',    icon=ICONMAINT, themeit=THEME3)
	...
	addDir ('[I]<< Zurueck zum Hauptmenu[/I]', icon=ICONMAINT, themeit=THEME2)
	...
	if type == 'gui':
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KOVI schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]KOVI schliessen[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Dieses Build [COLOR %s]%s[/COLOR] gibt es mit [COLOR %s]%s[/COLOR] verschiedenen Themen" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Willst du es jetzt installieren?[/COLOR]", yeslabel="[B][COLOR lime]Installiere Theme[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
...
def thirdPartyInstall(name, url):
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KOVI schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR lime]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR red]KOVI schliessen[/COLOR][/B]")
	if todo == 1: wiz.reloadFix()
	else: wiz.killxbmc(True)
