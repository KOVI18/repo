	...
	if len(match) > 0:
		...
		if version > BUILDVERSION:
			if DISABLEUPDATE == 'false':
				wiz.log("[Prüfe auf Updates] [Installierte Version: %s] [Aktuelle Version: %s] Offne Update Fenster" % (BUILDVERSION, version), xbmc.LOGNOTICE)
				notify.updateWindow(BUILDNAME, BUILDVERSION, version, icon, fanart)
			else: wiz.log("[Prüfe auf Updates] [Installierte Version: %s] [Aktuelle Version: %s] Offne Update Fenster deaktiviert" % (BUILDVERSION, version), xbmc.LOGNOTICE)
		else: wiz.log("[Prüfe auf Updates] [Installierte Version: %s] [Aktuelle Version: %s]" % (BUILDVERSION, version), xbmc.LOGNOTICE)
	else: wiz.log("[Prüfe auf Updates] FEHLER: Kann die Build Version nicht in der Build Text Datei finden", xbmc.LOGERROR)
...
def checkSkin():
	...
	if not DEFAULTSKIN == '':
		if os.path.exists(os.path.join(ADDONS, DEFAULTSKIN)):
			if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es scheint der Skin wurde zuruck gesetzt auf [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, SKIN[5:].title()), "Willst du den Skin zuruck setzten auf:[/COLOR]", '[COLOR %s]%s[/COLOR]' % (COLOR1, DEFAULTNAME)):
	...
	if DEFAULTSKIN == '':
		...
		if len(skinlist) > 0:
			if len(skinlist) > 1:
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es scheint der Skin wurde zuruck gesetzt auf [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, SKIN[5:].title()), "Willst du eine Liste aller Skins sehen?[/COLOR]"):
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es scheint der Skin wurde zuruck gesetzt auf [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, SKIN[5:].title()), "Willst du den Skin zuruck setzten auf:[/COLOR]", '[COLOR %s]%s[/COLOR]' % (COLOR1, skinname[0])):
					gotoskin = skinlist[0]
					gotoname = skinname[0]
				else: wiz.log("Skin was not reset", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true')
	...
	if gotoskin:
		...
		else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Skin Wechsel Timed Out![/COLOR]' % COLOR2)
...
if not ADDONID == path[1]: DIALOG.ok(ADDONTITLE, '[COLOR %s]Bitte stellle sicher, das der Plugin Ordner identisch ist mit ADDON_ID.[/COLOR]' % COLOR2, '[COLOR %s]Plugin ID:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, ADDONID), '[COLOR %s]Plugin Ordner:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, path)); wiz.log("[Path Check] ADDON_ID und Plugin Ordner nicht identisch. %s / %s " % (ADDONID, path))
...
if AUTOINSTALL == 'Yes' and not os.path.exists(os.path.join(ADDONS, REPOID)):
	...
	if workingxml == True:
		...
		if len(ver) > 0:
			...
			if workingrepo == True:
				DP.create(ADDONTITLE,'Downloade Repo...','', 'Bitte Warten')
				...
				wiz.LogNotify("[COLOR %s]Repo Installations Fehler[/COLOR]" % COLOR1, "[COLOR %s]Zip URL nicht korrekt![/COLOR]" % COLOR2)
				wiz.log("[Auto Install Repo] Was unable to create a working url for repository. %s" % workingrepo, xbmc.LOGERROR)
		else:
			wiz.logUngung URL for Repo Zip", xbmc.LOGERROR)
	else: 
		wiz.LogNotify("[COLOR %s]Repo Installations Fehler[/COLOR]" % COLOR1, "[COLOR %s]Addon.xml Dtei nicht korrekt![/COLOR]" % COLOR2)
		wiz.log("[Auto Install Repo] Unable to read the addon.xml file.", xbmc.LOGERROR)
elif not AUTOINSTALL == 'Yes': wiz.log("[Auto Install Repo] Nicht aktiviert", xbmc.LOGNOTICE)
